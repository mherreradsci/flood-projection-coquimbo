#!/usr/bin/env bash
# Wrapper para cron/systemd: corre 04_proyectar.py --fuente gfs --config config_atacama.yaml con log por corrida.
...
exec 9>"$LOGDIR/.proyeccion-atacama.lock"
...
"$REPO/.venv/bin/python" scripts/04_proyectar.py --fuente gfs --config config_atacama.yaml
